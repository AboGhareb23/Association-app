...
export default function Association({ association }) {
    ...
    const { data, setData, patch, processing, reset, errors } = useForm({
        ...
        total_months: association.total_months,
        role_price: association.role_price,
    });
 
    const submit = (e) => {
        e.preventDefault();
        patch(route('associations.update', association.id), { onSuccess: () => setEditing(false) });
    };
            ...
            <div className="flex-1">
                <div className="flex justify-between items-center">
                        ...
                        <Dropdown>
                            ...
                            <Dropdown.Content>
                                ...
                                <Dropdown.Link as="button" href={route('associations.destroy', association.id)} method="delete">
                                    حذف
                                </Dropdown.Link>
                            </Dropdown.Content>
                        </Dropdown>
                ...
                {editing
                    ? <form onSubmit={submit}>
                        ...
                        className="mt-4 w-full bg-gray-200 text-gray-900 border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm" />
                        <input value={data.role_price} onChange={e => setData('role_price', e.target.value)} 
                        className="mt-4 w-full bg-gray-200 text-gray-900 border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm" />
